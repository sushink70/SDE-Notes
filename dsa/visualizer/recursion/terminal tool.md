...
1. ✅ Fixed `ValueError` in `print_stats()` when frames is empty
2. ✅ Fixed `configure()` to update settings without replacing tracer instance
3. ✅ Added empty check in `export_call_tree()`
4. ✅ All decorators now properly reference the global `tracer` instance